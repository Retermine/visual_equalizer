	...
	for(i=0;i<1280;i++)
	{
		for(j=0;j<1024;j++){
			if(j>barpos[i*FFT_MAX_NUM_PTS/2/1280])
			{
				...
				eq_pattern[i][j]=color_pattern[i];
			}else{
...
void set_colorpattern()
{
	...
	for(i=0;i<1280;i++)
	{
		...
		{
			red=0;
			green=1-(float)(i-427)/426;
			blue=1 - green;
		}else if(i<1280&&i>=853)
		{
			green=0;
			blue=1-(float)(i-853)/426;
			red=1 - blue;
		}
		R = red*0xff;
		G = green*0xff;
		B = blue*0xff;
		color_pattern[i] = (R<<16)|(G<<8)|B;
	}
}
void refresh_frame()
{
